...
router.post('/create-account', async (req, res) => {
    ...
    user.password = await brycpt.hash(user.password, salt);
...
router.post('/sign-in', async(req, res)=>{
    ...
    if(!validPassword) return res.status(400).send('Invalis email or password');
    const token = user.generateAuthToken();
    express.response = {'token':token, 'email':user.email, 'memos':user.memos};
